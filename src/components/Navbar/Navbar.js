...
let _pages = ["", "products", "blog", "about", "contact", "my account"];
const NavList = _pages.map((item) => {
	return (
		<Link
			to={`/${item.replace(" ", "_")}`}
			key={item}
			className={
				item !== "my account" ? "nav-links-item" : "nav-links-item my-account"
			}
		>
		...
		<BackgroundStyle defaultBcg={background ? background : defaultBcg}>
			<div className="general-navbar">
				<nav className="center navbar">
					...
					<div
						className={
							navState ? "links nav-mobile-container" : "links links-container"
						}
					>
						<ul>{NavList}</ul>
					</div>
